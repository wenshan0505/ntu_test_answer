...
train_data_loader1 = DataLoader(dataset=train_data, shuffle=False,sampler = train_sampler1,batch_size= 100)  # 使用加载器加载，可以降低内存的消耗
train_data_loader2 = DataLoader(dataset=train_data, shuffle=False,sampler = train_sampler2,batch_size=batch_sizeof0)  # 使用加载器加载，可以降低内存的消耗
